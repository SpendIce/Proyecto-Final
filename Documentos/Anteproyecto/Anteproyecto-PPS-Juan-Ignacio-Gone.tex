...
Los factores cr\'iticos de \'exito (FCE) son las condiciones cuya presencia es necesaria ---aunque no suficiente--- para que el Proyecto alcance sus objetivos. La Tabla~\ref{tab:factores-exito} los enuncia junto con la estrategia de aseguramiento correspondiente. Cada FCE se controla con las m\'etricas anteriores y con los gates de calidad definidos en la Secci\'on~15.
...
\begin{longtable}{L{0.22\textwidth}L{0.35\textwidth}L{0.35\textwidth}}
  ...
  \label{tab:factores-exito}\\
    ...
    FCE-4: Soberan\'ia de datos & Ning\'un dato institucional sensible puede enviarse a servicios de \ac{LLM} externos. El incumplimiento invalida el sistema en un contexto de defensa nacional y vulnera la Ley N\textsuperscript{o}~25.326. & Uso de Ollama como inferencia base. Auditor\'ia de flujos de datos en cada release. Cualquier alternativa externa queda restringida a contingencia aprobada, datos anonimizados y registro expl\'icito. \\
    ...
    FCE-6: Disponibilidad funcional de A2--A5 & El Agente~1 act\'ua como nodo de comunicaci\'on institucional en S2: transforma el contenido producido por los dem\'as agentes en comunicaciones institucionales. Si A2--A5 no producen insumos utilizables al momento de la integraci\'on, la funci\'on de comunicaci\'on integrada queda sin contenido que distribuir, reduciendo el alcance a la operaci\'on aut\'onoma de S1. & Definici\'on de contratos de interfaz con mocks est\'aticos que permiten desarrollar y probar la comunicaci\'on integrada independientemente del avance de cada agente fuente. Integraci\'on real progresiva en S2 seg\'un disponibilidad confirmada de cada agente. \\
    FCE-7: Evidencia y trazabilidad temprana & La defensa acad\'emica depende de demostrar qu\'e se gener\'o, desde qu\'e insumo, con qu\'e estado, qui\'en valid\'o y qu\'e evidencia queda disponible. & Definir desde S1 un esquema de log con \texttt{id\_solicitud}, estado, validador, output y evidencia. Mantener \'indice de evidencias por \ac{HU} y por gate. \\
    FCE-8: Gobierno por estados & El avance no debe declararse por porcentaje subjetivo, sino por estados verificables de \ac{HU}, defectos y gate. & Usar estados de \ac{HU}: preparada, en implementaci\'on, en prueba, en validaci\'on \ac{SEU}, cerrada por \ac{DoD}, observada o bloqueada. Ninguna \ac{HU} se cierra sin \ac{DoD} y evidencia. \\
    FCE-9: Validaci\'on formal y responsables confirmados & Sin titular y suplente de validaci\'on por tipo de pieza, el \ac{HITL} queda informal y no defendible. & Confirmar responsables por \ac{HU}; registrar aprobaciones, observaciones y rechazos en canal formal; bloquear salidas oficiales si no hay validador. \\
    FCE-10: Despliegue progresivo y controlado & Saltar de documentaci\'on a operaci\'on real aumenta riesgo institucional y t\'ecnico. & Desplegar por etapas: documental, local controlado, sandbox Workspace, piloto \ac{SEU} limitado y release candidata \ac{TRL}~4. Cada etapa exige go/no-go y rollback definidos. \\
    \bottomrule
\end{longtable}
}
