...
Agente de comunicaci\'on institucional, automatizaci\'on de contenido, gacetillas universitarias, redes sociales, Large Language Models (LLM), LangChain, Ollama, Google Workspace, extensi\'on universitaria, Business Process Management (BPM), Technology Readiness Level (TRL), CONEAU, c\'odigo abierto, soberan\'ia de datos, arquitectura multiagente, human-in-the-loop, ciberseguridad desde el principio.
...
\begin{description}
  ...
  \item[LangChain:] framework de c\'odigo abierto para orquestar cadenas de llamadas a modelos de lenguaje, incluyendo gesti\'on de prompts, memoria y herramientas.
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item \textbf{Dise\~no:} especificaci\'on funcional del Agente~1, dise\~no de prompts y cadenas LangChain, definici\'on de plantillas institucionales con la \ac{SEU}, dise\~no de flujos de validaci\'on humana.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l>{\raggedright\arraybackslash}Xl}
    ...
    Orquestaci\'on LLM      & LangChain                                 & MIT \\
...
\noindent\textbf{Nota sobre evoluci\'on de infraestructura:} en el Semestre~1 (TRL~3) se opera con un stack m\'inimo: servidor local, Docker/Compose, Ollama, LangChain, FastAPI y Google Workspace. En el Semestre~2 (TRL~4) se incorporan Google Apps Script para triggers autom\'aticos, PostgreSQL para persistencia de logs y estado, y Celery~+~Redis para tareas asincr\'onicas. Esta evoluci\'on progresiva minimiza la complejidad inicial y permite validar el n\'ucleo de generaci\'on de contenido antes de agregar capas de automatizaci\'on.
...
El stack tecnol\'ogico seleccionado se compone de herramientas maduras con amplia adopci\'on en la industria. Los modelos de lenguaje de c\'odigo abierto pueden ejecutarse en hardware de consumo sin GPU dedicada, lo que los hace compatibles con los servidores institucionales existentes. LangChain provee abstracciones probadas para la construcci\'on de cadenas de generaci\'on de contenido, con soporte nativo para prompts con variables, memoria de contexto y salidas estructuradas. La integraci\'on con Google Workspace a trav\'es de su API y Apps Script es un patr\'on bien documentado y ampliamente utilizado. Los mayores riesgos t\'ecnicos son la calidad del contenido generado (mitigado con validaci\'on humana obligatoria), la inyecci\'on de prompts sobre datos no confiables (mitigada con sanitizaci\'on, separaci\'on de instrucciones y revisi\'on de salidas) y la latencia de generaci\'on (objetivo: $<$~30 segundos por pieza).
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{l>{\raggedright\arraybackslash}Xrr}
    ...
    Software base   & Stack open-source (Python, FastAPI, LangChain, Ollama, Docker, PostgreSQL, Git) & 0 & Gratuito \\
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|l|X|}
    ...
    Servidor institucional existente. Google Workspace institucional. Stack open-source (LangChain, Ollama, FastAPI). Plantillas institucionales definidas con la SEU. Conocimiento del equipo de desarrollo. \\
    \hline
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[bloque, fill=blue!15] (a1) at (10.5, 0) {Agente~1\\(LangChain + LLM)};
...
\begin{enumerate}[label=\textbf{Paso \arabic*:}]
  ...
  \item \textbf{Generaci\'on de contenido:} el Agente~1 construye una cadena LangChain con el prompt correspondiente a la plantilla de gacetilla institucional, le pasa los datos como variables y solicita la generaci\'on al modelo de lenguaje de c\'odigo abierto ejecutado localmente v\'ia Ollama. El proceso tarda menos de 30 segundos.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[font=\small\bfseries] at (0.9, 1.1) {LangChain};
